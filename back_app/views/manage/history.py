...
class HistoryView(ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        categories = []
        rows = History.objects.values('category').distinct()
        for row in rows:
    ...
    def post(self, request):
        sql = f"FROM back_history h LEFT JOIN auth_user u ON h.user_id = u.id WHERE 1"
        ...
        for param_key in request.POST.keys():
            ...
            if param_key == 'sort[field]':
                ...
                sql += " OR CAST(action AS CHAR CHARACTER SET utf8) COLLATE utf8_general_ci LIKE '%%" + search + "%%'"
                sql += " OR CAST(comment AS CHAR CHARACTER SET utf8) COLLATE utf8_general_ci LIKE '%%" + search + "%%'"
        ...
        with connection.cursor() as cursor:
            ...
            if sort_field not in ['category', 'action', 'comment', 'created_at', 'username']:
                sort_field = 'h.id'
                sort_direction = 'desc'
            ...
            # print(request.POST)
            # print(f'SELECT * {sql}')
            cursor.execute(f'SELECT category, action, comment, created_at, user_id, username {sql}')
            ...
            for row in rows:
                ...
                item['comment'] = row[2]
                if row[3]:
                    item['created_at'] = row[3].strftime('%m/%d/%Y')
                else:
                    item['created_at'] = ''
                item['user_id'] = row[4]
                item['username'] = row[5]
